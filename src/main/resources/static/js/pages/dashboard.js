...
// ===================================================
//  CHATBOT AI – Trợ lý HR thông minh
// ===================================================
let chatHistory = [];
...
function sendChat() {
    ...
    $.ajax({
        ...
        error: function(xhr) {
            $('#chat-typing').remove();
            if (xhr.status === 401) {
                appendMessage('Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại.', 'bot');
            } else {
                const err = xhr.responseJSON?.error || 'Có lỗi xảy ra, vui lòng thử lại.';
                appendMessage('Xin lỗi: ' + err, 'bot');
            }
        }
    });
}
...
function appendMessage(text, role) {
    const msgArea = $('#chat-messages');
    const div = $('<div>').addClass('chat-msg ' + role);
    ...
    msgArea.find('.chat-welcome-msg').remove();
    msgArea.append(div);
    msgArea.scrollTop(msgArea[0].scrollHeight);
}
